...
function returnTen(str){
  let Array = [];
  let arr = str.split ('');
  Array =arr.splice( arr.length - 10);
  return Array;
}
...
const typeNum = (arr) => {
  let Array= arr.filter(element=> typeof (element) === 'number');
  return Array;
};
...
const containsAnd = (arr) => {
  let Array = arr.filter(item => item.includes('and') === true);
  return Array;
};
...
const filterStringsWithVowels = (arr) => {
  let Array = arr.filter(element => element.match(/[aeiou]/g));
  return Array;
};
