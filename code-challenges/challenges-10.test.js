...
const count = (target, input) => {
  let acc = 0;
  let Array = input.filter(element=>{
    element.filter(val =>{ if(val === target){
      acc = acc+1;
    }});
  });
  return acc;
...
const divisibleByFiveTwoToThePower = (input) => {
  let result =input.map(element => element.filter(val => val % 5 === 0 && Number(val) === val).map(newValue => Math.pow(2, newValue)));
...
let findMaleAndFemale = (data) => {
  let Array=data.filter(names => {
    if(names.gender ==='female' || names.gender==='male' )
    {
      return names.name;
    }
  });
  let DataArray = Array.map(p=>
  {
    return p.name;
  });
  let DataArray2 = DataArray.join( ' and ' );
  return DataArray2;
};
